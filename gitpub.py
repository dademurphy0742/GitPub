...
def load_last_scan():
    ...
    for latest in reversed(files):
        ...
        try:
            ...
            console.print(f"[yellow][!] Skipping corrupted scan file: {latest}[/yellow]")
    return []
...
# ── Display Repos ───────────────────────────────────
def display_repos(title, repos, new_repos=[], newly_cross=[]):
    if not repos:
        return  # skip empty tables
    for r in repos[:10]:
        table = Table(box=box.SIMPLE, expand=True)
        table.add_column("Field", style="bold cyan", no_wrap=True)
        table.add_column("Value", style="white", no_wrap=False, overflow="fold", justify="left", min_width=30)
...
def report(data):
    ...
    console.print("\n[bold underline magenta]=== MERGED SIGNAL REPORT ===[/bold underline magenta]\n")

    console.print("[bold green][HOT TERMS][/bold green]")
    ...
    display_repos("CROSS-SIGNAL REPOS (appear in multiple queries)", cross, new_repos, newly_cross)
    display_repos("NEW REPOS SINCE LAST SCAN", new_repos, new_repos, newly_cross)
    display_repos("NEWLY CROSS-SIGNAL REPOS", newly_cross, new_repos, newly_cross)

    console.print("\n[bold green][TOP INTERESTING] (Trendiness Scored)[/bold green]")
    scored = []
    ...
    for r, score in scored[:10]:
        ...
        table.add_row("📛 Full Name", full_name)
        table.add_row("⭐ Stars", str(r.get("stars", 0)))
        table.add_row("🔥 Trendiness", str(score))
        table.add_row("🌐 Sources", ", ".join(r.get("sources", [])) or "None")
        table.add_row("🏷️ Tags", ", ".join(r.get("tags", [])) or "None")
        table.add_row("🔗 URL", url)
        table.add_row("📝 Description", r.get("desc", ""))
    ...
    console.print("\n[bold green][SCAN TIME][/bold green]")
