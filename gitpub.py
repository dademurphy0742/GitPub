...
def report(data):
    ...
    console.print(Panel(hot_terms, title="[bold green]HOT TERMS[/bold green]", expand=False))

    # Patterns
    pattern_text = "\n".join(f"[magenta]- {p} ({c})[/magenta]" for p, c in patterns.most_common(5))
    console.print(Panel(pattern_text, title="[bold green]PATTERNS[/bold green]", expand=False))
    ...
    def display_repos(title, repos):
        for r in repos[:10]:
            ...
            console.print(Panel(table, title=f"[bold blue]{title}[/bold blue]", expand=False))
